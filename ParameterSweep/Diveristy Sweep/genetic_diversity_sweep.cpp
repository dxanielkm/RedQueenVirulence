...
int main() 
{
    ...
    // We'll log everything: s, N, b, gamma, theta, delta, Average_Virulence
    ...
    for (int r = 0; r < num_random_sets; ++r)
    {
        ...
        double b_rand     = dist_b(gen);
        double th_rand    = dist_th(gen);
        double de_rand    = dist_de(gen);
        double ga_rand    = dist_ga(gen);

        // For each random parameter set, loop over s and N
        for (int N_val : N_values) {
            for (double s_val : s_values) 
            {
                std::cout << "Random Set " << r << " | s=" << s_val 
                          << ", N=" << N_val 
                          << ", b=" << b_rand 
                          << ", gamma=" << ga_rand
                          << ", theta=" << th_rand
                ...
                // Initialize infected and parasite for trait 0 only
                int idx      = N_val;
                int trait    = 0;
                ...
                for (int step = 1; step < evolSteps; ++step) {
                    ...
                    // Mutation Logic
                    // ------------------
                    // For each virulence trait, sum infected across all Iab
                    ...
                    for (int t_index = 0; t_index < numTraits; ++t_index) {
                        ...
                        int Iab_start2 = idx_t;
                        double sum_Iab = 0.0;
                        for (int i = 0; i < N_val * N_val; ++i) {
                            sum_Iab += y[Iab_start2 + i];
                        }
                    ...
                    if (total_infected == 0.0) {
                        // Extinction
                        std::cout << "Extinction at step " << step << ", skipping rest.\n";
                        Yout.resize(step + 1);
                        Yout[step] = y;
                        break;
                    }
                ...
                if (total_infected_final > 0.0) {
                    for (int t_index = 0; t_index < numTraits; ++t_index) {
                        average_virulence += 
                            (I_total_final[t_index] / total_infected_final) * params.myAlpha[t_index];
                    }
                }

                // Write to CSV: s, N, b, gamma, theta, delta, average_virulence
                csvfile << s_val << ","
                        << N_val << ","
                        << b_rand << ","
                        << ga_rand << ","
                ...
                std::cout << "Finished => s=" << s_val << ", N=" << N_val 
                          << " | Average Virulence= " << average_virulence << std::endl << std::endl;
            }
        }
    }
